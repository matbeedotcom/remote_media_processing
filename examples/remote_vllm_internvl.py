#!/usr/bin/env python3
"""
Example of using the VLLMNode to run the OpenGVLab/InternVL2-2B model
...
3.  **Run this script:**
    $ python examples/remote_vllm_internvl.py
"""
...
async def main():
    ...
    logger.info("--- Running Remote VLLM InternVL-2B Pipeline ---")

    # For InternVL, we must use the chat template. The node will handle
    # loading the tokenizer and applying the template on the server side.
    ...
    # using a chat template: a dictionary with a 'messages' key.
    ...
    pipeline = Pipeline(
        [
            ...
            TakeFirstFrameNode(),
            transform_for_chat,
            remote_config(internvl_vllm_node),
            PrintResponseStream(),
        ]
    )
